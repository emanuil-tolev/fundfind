...
def identify(therest=''):
    ...
    if q:
        answers = fundfind.dao.Identifier.identify(q=q)
        ...
        if JSON:
            ...
            for a in answers:
                ...
                t = fundfind.dao.Test.query(q=a['name'])
                if t['hits']['total'] != 0:
                    t = t['hits']['hits']
                    for hit in t:
...
def browse():
    # tests = fundfind.dao.Test.query(sort=[{"name":"asc"}]) # get all the tests # ES query with sorting fails for tests which have spaces in their names
    tests = fundfind.dao.Test.query() # get all the tests
    tests = tests['hits']['hits']
    
    descs = fundfind.dao.Description.query() # get all the descriptions

...
class IOManager(object):
    def __init__(self, results, args={}, path=""):
        ...
        self.args = args
        self.path = path
        self.config = fundfind.config.Config()
    ...
    def get_keys(self,rectype='Regex'):
        if rectype == 'Record':
            res = fundfind.dao.Record.get_mapping()
            which = 'record'
        elif rectype == 'Collection':
            res = fundfind.dao.Collection.get_mapping()
            which = 'collection'
    ...
    def get_sort_fields(self, rectype='Record'):
        ...
        for item in fields:
            try:
                if rectype == 'Record':
                    fundfind.dao.Record.query(sort={item+self.config.facet_field: {"order":"asc"}})
                elif rectype == 'Collection':
                    fundfind.dao.Collection.query(sort={item: {"order":"asc"}})
